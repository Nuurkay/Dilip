...
class CreateMachineUser(models.TransientModel):
    ...
    link_if_exists = fields.Boolean(string='Link User If Exists', help="If partner name is found on machine, it will update User ID on partner card")
    ...
    def create_user(self):
                    ...
                    if record.device_id:
                        ...
                        if found_user:
                            ...
                            try:
                                ...
                                error = "Could not create partner {0} with deviceId: {1}. Exception occurred: {2}".format(record.name, record.device_id, ex.name)
                                log.info(error)
                                errors.append(error)
                                continue  
                    else:
                        try:
                            ...
                            error = "Could not create partners {0} . Exception occurred: {1}".format(record.name,  ex.name)
                            log.info(error)
                            errors.append(error)
                            continue  
        ...
        errors.insert(0, "%s Partners has been created successfully.\n" % (created_count) )
...
class DeleteMachineUser(models.TransientModel):
    ...
    def delete_user(self):
        ...
        conn = False
        #changed employee_id field to partner_id
        try:
            conn, _ = self.machine_id.connect()
 
            if conn:
                found_user = self.machine_id.check_user_id_availabilty(self.partner_id.device_id, conn)
                if not found_user:
                   raise UserError('User does not exist on machine')
                else:
                    res = conn.delete_user(user_id=self.partner_id.device_id)
                    self.partner_id.device_id = False
                    self.partner_id.message_post(body="User {0} is linked on machine {1} with Device Id {2}".format(self.partner_id.name, self.machine_id.name, self.partner_id.device_id))
        except Exception as ex:
