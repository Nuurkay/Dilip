...
class ZkMachine(models.Model):
    ...
    auto_create_partner = fields.Boolean("Auto create partner", help="Automatically create the partner on Odoo if not found", default=False)
    ...
    def cron_download(self):
        # machine = self.env['azk.machine']
        machines = self.env['ank.machine'].search([])
    ...
    def download_attendance(self):
        ...
        PartnerAttendance = self.env['partner.attendance']
        # PartnerAttendance.create({'partner_id': 66, 'check_in': "2024-10-24 15:30:00", 'check_out': "2024-10-24 19:30:00"})
        conn = None
        ...
        try:
            ...
            if conn:
                ...
                partners_by_device_id = dict(map(lambda e: (e.device_id, e), self.env['res.partner'].search([('device_id', 'in', list(map(lambda u:u.user_id, machine_users)))])))
                
                total_attendance_rec, total_users = len(lst_attendance), len(machine_users)
    
                partners_with_attendance = {} #just cache to mininimize queries. Contains partner ID
                ...
                for a_rec in sorted(lst_attendance, key=lambda a: a.timestamp):
                    try:
                        ...
                        local_tz = pytz.timezone(tz or "UTC")
                        ...
                        partner = partners_by_device_id.get(emp_device_id)
                        if not partner:
                            #try to look it in the DB and if found then update the device id
                            partner = self.find_or_create_partner(users_by_id.get(emp_device_id)) if users_by_id.get(emp_device_id) else False
                            partners_by_device_id[emp_device_id] = partner 
                        ...
                        if partner:
                            ...
                            if not duplicate_atten_ids:
                                ...
                                punch_type, _, _ = ZkMachine.resolve_punchtype(a_rec.timestamp, partner)
                                previous_check_in = PartnerAttendance.search([('partner_id', '=', partner.id), ('check_out', '=', False)], limit=1)                                
                                if punch_type == 'checkin':
                                    #get from cache if employee has attendance
                                    if partner.id in partners_with_attendance:
                                        emp_has_attendance_rec = partners_with_attendance[partner.id]
                                    else:
                                        emp_has_attendance_rec = 1 if PartnerAttendance.search([('partner_id', '=', partner.id)], limit=1) else 0
                                        partners_with_attendance[partner.id] = emp_has_attendance_rec
                                          
                                    #if doesn't have any attendance record or last all are checked out then this is a checkin
                                    if not emp_has_attendance_rec:
                                        PartnerAttendance.create({'partner_id': partner.id, 'check_in': atten_time})
                                        partners_with_attendance[partner.id] = 1
                                        total_checkins += 1
                                    #if has checkin records but last one was not a checkin
                                    elif not previous_check_in:
                                        if not PartnerAttendance.search([('partner_id', '=', partner.id), ('check_in', '=', atten_time)], limit=1):
                                            PartnerAttendance.create({'partner_id': partner.id, 'check_in': atten_time})
                                        total_checkins += 1
                                            ...
                                            PartnerAttendance.create({'partner_id': partner.id, 'check_in': atten_time})
                                            total_checkins += 1
                                    elif previous_check_in.check_in < atten_time_ts and not PartnerAttendance.search([('partner_id', '=', partner.id), ('check_out', '=', atten_time)], limit=1):
                                    ...
                                    if previous_check_in and previous_check_in.check_in < atten_time_ts and not PartnerAttendance.search([('partner_id', '=', partner.id), ('check_out', '=', atten_time)], limit=1):
                                        previous_check_in.write({'check_out': atten_time})
                                        total_checkouts += 1
                                    else:
                                        import_status = 'skipped'
...
#                                     total_checkins += 1
                                ...
                                AZKAttendance.create({'partner_id': partner.id,
                                                      'device_id': emp_device_id,
                                                      'attendance_type': str(a_rec.status),
                                                    #   'attendance_type': str(a_rec.get("status")),
    ...
    @staticmethod
    def resolve_punchtype(punch_time, partner):
        """
        ...
        cal_attendance = partner.resource_calendar_id.attendance_ids.filtered(lambda a: a.dayofweek == str(punch_time.weekday()))
        #if not attendance for given day then get the first one
        if not cal_attendance:
            cal_attendance = partner.resource_calendar_id.attendance_ids.filtered(lambda a: a.dayofweek == partner.resource_calendar_id.attendance_ids[0].dayofweek)
    ...
    def find_or_create_partner(self, user):
        """
        ...
        if partner:
            partner.write({'device_id': emp_device_id})
            
        if not partner and self.auto_create_partner:
            partner = self.env['res.partner'].create({'device_id': emp_device_id, 'name': user.name})
