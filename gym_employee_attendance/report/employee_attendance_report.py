...
class PartnerAttendanceReport(models.AbstractModel):
    _name = "report.gym_employee_attendance.employee_attendance_report"
    ...
    def _get_data(self, context):
        ...
        for rec in context.get("employee_ids"):
            employee_attendance_ids = self.env["employee.attendance"].search(
                [
            ...
            employee_id = self.env["hr.employee"].browse(rec)
            if employee_attendance_ids:
                attendance.update({employee_id: employee_attendance_ids})
        res.update({"to_date": to_date, "employee_attendance_ids": attendance})
        return res
    ...
    def _get_report_values(self, docids, data=None):
        ...
        report = self.env.ref("gym_employee_attendance.action_employee_report")
